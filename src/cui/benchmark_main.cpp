...
namespace {
  size_t CountVertices(const vector<pair<int, int> > &es){
  ...
  void OutputLCCSize(const vector<vector<int>  > &edge_groups, const vector<pair<int, int> > &es){
    size_t lcc_es = 0;
    size_t lcc_vs = 0;
...
int main(int argc, char *argv[])
{
  ...
  JLOG_OPEN("results") JLOG_PUT_BENCHMARK("total_time"){
    ...
    for (int q = 0; q < FLAGS_num_trials; q++) {
      ...
      spanning_centrality::EstimateEdgeCentrality(es, FLAGS_num_samples);
